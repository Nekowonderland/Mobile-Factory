...
function EL:findEntity()
	...
	if nearestEnt ~= nil and (storage.entsTable[nearestEnt.unit_number] ~= nil or storage.objectsTable[nearestEnt.unit_number] ~= nil) and ((_mfEnergyStructures[nearestEnt.name] ~= nil and _mfEnergyStructures[nearestEnt.name].canAccept == true) or string.match(nearestEnt.name, "MobileFactory") or string.match(nearestEnt.name, "EnergyLaser")  ) then
		-- Get the Object --
