...
function QL:findEntity()
	...
	if nearestEnt ~= nil and (storage.entsTable[nearestEnt.unit_number] ~= nil or storage.objectsTable[nearestEnt.unit_number] ~= nil) and ((_mfQuatronStructures[nearestEnt.name] ~= nil and _mfQuatronStructures[nearestEnt.name].canAccept == true) or string.match(nearestEnt.name, "MobileFactory") or string.match(nearestEnt.name, "QuatronLaser") ) then
		-- Get the Object --
		local obj = storage.entsTable[nearestEnt.unit_number] or storage.objectsTable[nearestEnt.unit_number]
		-- Same target --
